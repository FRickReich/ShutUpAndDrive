...
namespace Game.PopulationManager
{
    public class PopulationManager : MonoBehaviour
    {
        ...
        public void CreateObject(Vector3 tilePosition, Transform currentTile)
        {
            ...
            if(tile.active == true && tile.creator == true)
            {
                ...
                {
                    tile.active = false;
                    GameObject newObject = Instantiate(testCharacterPrefab, RandomPosition(tilePosition), Quaternion.identity);
                }
                if (tile.tileType == TileType.GROUND && tile.VehicleSpawnable == true)
                {
                    tile.active = false;
                    GameObject newObject = Instantiate(testVehiclePrefab, RandomPosition(tilePosition), Quaternion.identity);
                }
            }
