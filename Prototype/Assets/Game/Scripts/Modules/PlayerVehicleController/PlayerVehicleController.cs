...
namespace Game.Controllers
{
	...
	public class PlayerVehicleController : MonoBehaviour
	{
        ...
        private void OnEnable()
        {
            ...
            inputControls.Vehicle.HandBrake.performed += context => VehicleHandbrake(context.ReadValue<float>());
            inputControls.Vehicle.Boost.performed += context => VehicleBoost(context.ReadValue<float>());
            inputControls.Vehicle.Steer.performed += context => VehicleSteer(context.ReadValue<Vector2>());
            //inputControls.Vehicle.Accelerate.performed += context => VehicleAccelerate(context.ReadValue<float>());
            //inputControls.Vehicle.Brake.performed += context => VehicleBrake(context.ReadValue<float>());

            inputControls.Enable();
        }

		private void VehicleHandbrake(float v)
		{
			handbrakeInput = v;
		}

		private void VehicleBoost(float v)
		{
			boostInput = v;
		}

		private void VehicleSteer(Vector2 direction)
		{
			turnInput = direction.x;
		}

		private void VehicleBrake(float brakeValue)
		{
			brakeInput = brakeValue;
		}

		private void VehicleAccelerate(float accelerationValue)
		{
			throttleInput = accelerationValue;
		}
        ...
        void FixedUpdate () {
            ...
            // Get all the inputs!
            if (isPlayer) {
                //throttle = throttleInput - brakeInput;
                // Boost
                boosting = (boostInput > 0.5f);
                // Turn
                steering = turnInputCurve.Evaluate(turnInput) * steerAngle;
                // Dirft
                drift = driftInput > 0 && _rb.velocity.sqrMagnitude > 100;
                // Handbrake
